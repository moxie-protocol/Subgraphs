...
export function handleLock(event: Lock): void {
  ...
  if (event.params._buyer != Address.zero()) {
    ...
    let buyerPortfolio = getOrCreatePortfolio(event.params._buyer, event.params._subjectToken, event.transaction.hash, event.block, false)
...
export function handleLockExtended(event: LockExtended): void {
  let portfolio = getOrCreatePortfolio(event.params._user, event.params._subjectToken, event.transaction.hash, event.block, false)
