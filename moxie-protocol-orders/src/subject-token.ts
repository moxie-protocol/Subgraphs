...
export function handleTransfer(event: Transfer): void {
  ...
  if (mint) {
    ...
    summary.totalSubjectTokensIssued =
      summary.totalSubjectTokensIssued.plus(value)
  }
  ...
  if (burn) {
    ...
    summary.totalSubjectTokensIssued =
      summary.totalSubjectTokensIssued.minus(value)
  }
  summary.save()
  ...
  if (!mint) {
    let fromPortfolio = getOrCreatePortfolio(
      from,
      contractAddress,
    ...
    if (fromPortfolio.balance.gt(BigInt.zero())) {
      protcolTokenInvestedDiff = fromOldProtocolTokenInvested
    ...
    if (
      ...
      // making sure that both  users are not staking 
    ...
    fromUser.protocolTokenInvested = fromUser.protocolTokenInvested.minus(
      protcolTokenInvestedDiff
    )
  ...
  if (!burn) {
    let toPortfolio = getOrCreatePortfolio(
      to,
      contractAddress,
      event.transaction.hash,
      event.block
    ...
    toPortfolio.protocolTokenInvested = toPortfolio.protocolTokenInvested.plus(
      protcolTokenInvestedDiff
    )
